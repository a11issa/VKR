...
def calculate_physics(H, P_pl, P_gr, T_zab, angle):
    rho_w = 1000
    g = 9.81

    # Считаем эквивалентную плотность по пластовому давлению
    rho_eq = (P_pl * 1e6) / (rho_w * g * H)

    # Добавляем абсолютный запас репрессии (~0.04 г/см³) вместо умножения,
    # чтобы избежать ложных ошибок на малых глубинах с аномальным давлением
    rho_0 = rho_eq + 0.04

    # Считаем плотность гидроразрыва (верхняя граница окна бурения)
    rho_max = (P_gr * 1e6) / (rho_w * g * H)
